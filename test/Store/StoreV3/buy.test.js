...
describe('StoreV3.buy', function () {
  ...
  it('buy: should buy product', async function () {
    ...
    const purchasesCountStart = await store.purchasesCount().then((v) => v.toString());

    const tx = await store.buy(product.id, account.address, {
      value: new bn(price).multipliedBy(2).toFixed(0),
      gasPrice: 0,
    });
    const purchaseId = new bn(purchasesCountStart).plus(1).toFixed(0);
    const purchase = await store.purchases(purchaseId);

    strictEqual(await store.purchasesCount().then((v) => v.toString()), purchaseId, 'Invalid end purchases count');
    strictEqual(purchase.id.toString(), purchaseId, 'Invalid purchase id');
    strictEqual(purchase.recipient.toString(), account.address, 'Invalid product recipient');
    strictEqual(purchase.product.toString(), product.id, 'Invalid product id');
    strictEqual(
    ...
    strictEqual(String(buyEvent.args.product), product.id, 'Event invalid product id');
    strictEqual(buyEvent.args.recipient, account.address, 'Event invalid recipient');
  });
